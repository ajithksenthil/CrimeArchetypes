...
Using structural causal modeling and do-calculus, we identified the Seeking$\rightarrow$Directing transition (``fantasy-to-action'') as the primary intervention target. Counterfactual simulations estimated early intervention could reduce harmful outcomes by 23--47\%. State space validation confirmed the four-state representation captures significant temporal structure ($p < 0.0001$), with Markov prediction improving accuracy by 12.6\% over marginal baselines.
...
We address these limitations by applying \textbf{Computational Psychodynamics} \citep{senthil2026comppsy}, a recently developed framework that models behavioral dynamics using principles from Active Inference and the Free Energy Principle \citep{friston2010free, parr2022active}. The framework provides:
...
\subsection{Mathematical Foundation: Steiner System $S(3,4,8)$}

The four-state space inherits guarantees from the Steiner system $S(3,4,8)$ \citep{senthil2026comppsy}:

\begin{enumerate}
    \item \textbf{Mutual exclusivity}: Each observation maps to exactly one state
    \item \textbf{Collective exhaustiveness}: Every possible observation is representable
    \item \textbf{Unique triple cover}: Coarse-graining preserves class boundaries
    \item \textbf{Uniform incidence}: Balanced priors across states
    \item \textbf{Minimal category count}: Smallest alphabet satisfying 1--4
\end{enumerate}

These properties ensure that the resulting Markov counts are clean, unbiased, and tractable.
...
\subsection{Rationale: Archetypal Reincarnation}

Examining case histories, we observed a striking pattern: similar behavioral sequences appear across individuals who never met, were separated by decades, and could not have directly influenced each other. We term this phenomenon \textit{archetypal reincarnation}---the recurrence of hierarchical behavioral patterns across human lifetimes.

This terminology is not merely metaphorical. Transfer entropy quantifies the degree to which one individual's life trajectory provides predictive information about another's. When $TE(A \rightarrow B) > 0$, individual $A$'s behavioral sequence reduces uncertainty about individual $B$'s future states, even though these individuals never interacted. Mathematically, this constitutes \textit{experiential influence across lifetimes}: the structure of lived experience in one life informs the structure of another.

\textbf{Archetypes}, in this framework, are the latent generative patterns that produce similar trajectories across individuals. When multiple unconnected individuals exhibit high mutual transfer entropy, they share an underlying archetypal structure---a common ``template'' that shapes how motivational states unfold over a life course. The term \textit{reincarnation} captures the empirical observation that these templates manifest repeatedly across different lives, as if the same experiential pattern were being re-instantiated in new individuals.

Computational Psychodynamics provides the mathematical tools to operationalize these concepts: \textbf{transfer entropy} measures directed predictive influence between life trajectories, while \textbf{network analysis} reveals the hierarchical structure of archetypal relationships.
...
where the sum is over all state combinations. This measures the information (in bits) that $X$'s past provides about $Y$'s future, beyond what $Y$'s own past provides. Sequences were phase-normalized (resampled to common length) to align ``life phases'' rather than absolute time.
...
%==============================================================================
\section{Study 4: Causal Modeling and Intervention Optimization}
%==============================================================================

\subsection{From Description to Prescription}

Studies 1--3 characterized patterns; Study 4 enables action. We develop a Structural Causal Model (SCM) that supports intervention reasoning via do-calculus \citep{pearl2009causality}.
...
We developed 14 evidence-based protocols grounded in RNR principles \citep{andrews2010RNR} and therapeutic frameworks:
...
Using Monte Carlo methods, we simulated counterfactual trajectories to estimate potential intervention effects. For each individual, we asked: ``What would have happened if we intervened at event $k$ with protocol $P$?'' These estimates are model-based projections derived from transition probability modifications, not empirical treatment effects.
...
\begin{table}[H]
\centering
\caption{Simulated counterfactual harm reduction by intervention timing.}
\begin{tabular}{llrrl}
\toprule
...
\begin{keyinsight}
Early intervention is 2--3$\times$ more effective than late intervention. Number Needed to Treat (NNT) doubles post-tipping point. Even late intervention has non-trivial effect (18.6\% reduction).
\end{keyinsight}
...
\begin{keyinsight}
The 4-state representation captures \textbf{highly significant temporal structure} in behavioral sequences (Tests 2--3, $p < 0.0001$). However, the specific cluster-to-state mapping is \textbf{not uniquely optimal} (Test 1, $p = 0.21$). This validates the \textit{dimensionality} of the representation while suggesting the mapping should be justified on theoretical/interpretability grounds rather than statistical optimality.
\end{keyinsight}
...
\begin{enumerate}
    \item \textbf{The 4-state space is appropriate}: Temporal structure exists and is captured ($p < 0.0001$)
    \item \textbf{Markov modeling is justified}: Conditioning on previous state substantially improves prediction (+12.6\%)
    \item \textbf{The specific mapping is conventional, not optimal}: The theoretical mapping is not statistically superior to alternatives
...
\begin{enumerate}
    \item \textbf{Behavioral fingerprints}: The four-state Markov chain captures individual differences in criminal career dynamics (Study 1)
    \item \textbf{Archetypal discovery}: Transfer entropy reveals shared generative structure (``reincarnation'') across unconnected individuals (Study 2)
    \item \textbf{Hierarchical classification}: COMPLEX/FOCUSED + 7 subtypes balances empirical validity with clinical utility (Study 3)
    \item \textbf{Intervention reasoning}: SCM + do-calculus enables counterfactual analysis and optimization (Study 4)
    \item \textbf{Methodological validation}: Rigorous comparison of theory-driven vs. data-driven state spaces establishes the framework's statistical validity (Study 5)
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Sample size}: $N = 26$ limits generalizability
    \item \textbf{Retrospective data}: Selection bias toward well-documented cases
    \item \textbf{LLM classification}: Potential for systematic bias
    \item \textbf{Intervention effects}: From literature, not this sample
    \item \textbf{Causal assumptions}: SCM requires correct graph structure
\end{itemize}
...
Beyond description, the framework enables prescription: identifying when interventions have maximum leverage, which protocols best disrupt harmful transitions, and what outcomes alternative histories might have produced. This represents a shift from ``who is dangerous'' to ``what can we do''---from risk assessment to intervention optimization.
...
Senthil, A. K. (2026). Computational psychodynamics: An ecological approach to computational behavioral modeling. Manuscript in preparation.
